...
function AccountManager() {
  ...
  */
  ...
  async function getTransactionByHash(txHash,account,xrplClient_) {
    // let _accountInfo = await xrplClient_.request({
    //   command: "account_tx",
    //   "account": connectedAddress,
    // });
    ...
    let txs=_accountInfo.result.transactions
    console.log("transactions are ", txs);
    let targetTx =null;
    
    for (let index = 0; index < txs.length; index++) {
      const item = txs[index];
      if(item.tx.hash==txHash) {
        console.log("target Tx := ",item.tx);
        return item.tx
      }
